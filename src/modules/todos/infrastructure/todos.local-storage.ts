...
export class TodosLocalStorage implements TodosOutput {
  ...
  addTodo({ todoTitle }: { todoTitle: string }): Promise<Todo[]> {
    return this.getTodos().then((todos: Todo[]) => {
      const isTodoExists: boolean =				todos.find((todo: Todo) => todo.title === todoTitle)
				!== undefined;
  ...
  toggleCompleteTodo({ todoTitle }: { todoTitle: string }): Promise<Todo[]> {
    return this.getTodos().then((todos: Todo[]) => {
      const modifiedTodos = [
        ...todos.map((todo: Todo) => (todo.title === todoTitle
          ? {
            ...todo,
            isDone: !todo.isDone,
						  }
      ...
      this.setLocalTodos(modifiedTodos);

      return Promise.resolve(modifiedTodos);
    });
  }

  removeTodo({ todoTitle }: { todoTitle: string }): Promise<Todo[]> {
    return this.getTodos().then((todos: Todo[]) => {
      const modifiedTodos = [...todos.filter((todo: Todo) => todo.title !== todoTitle)];
